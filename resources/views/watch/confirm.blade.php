        ...
        <div class="container">
            <div class="row">
                <div class="col-md-8 p-b-30">
                    {!! Form::open(['route' => 'cart.checkout', 'method' => 'post', 'class' => 'leave-comment', 'onsubmit' => 'return confirm("Are you sure?")']) !!}
                        <h4 class="m-text26 p-b-36 p-t-15">
                            @lang('message.your_infor')
                        </h4>
                        {!! Form::label('name', trans('message.name'), ['class' => 'col-form-label text-md-right']) !!}
                        <div class="bo4 of-hidden size15 m-b-20">
                            {!! Form::text('name', $customer->name, ['disabled', 'class' => 'sizefull s-text7 p-l-22 p-r-22']) !!}
                        </div>

                        {!! Form::label('phone', trans('message.phone'), ['class' => 'col-form-label text-md-right']) !!}
                        <div class="bo4 of-hidden size15 m-b-20">
                            {!! Form::text('phone', $customer->phone, ['disabled', 'class' => 'sizefull s-text7 p-l-22 p-r-22']) !!}
                        </div>

                        {!! Form::label('address', trans('message.address'), ['class' => 'col-form-label text-md-right']) !!}
                        <div class="bo4 of-hidden size20 m-b-20">
                            {!! Form::textarea('address', $customer->address, ['class' => 'dis-block s-text7 size20 bo4 p-l-22 p-r-22 p-t-13',
                            'rows' => config('custom.rows'), 'cols' => config('custom.cols'), 'disabled']) !!}
                        </div>
